    ...
    <section id="trainers" className="py-16 md:py-24 bg-white">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-lg max-w-3xl mx-auto">Work with one of our expert trainers or take a group class! Our instructors are highly trained professionals who are passionate about helping others reach their fitness goals.</p>
          <p className="text-secondary font-medium mt-4">*Personal training sessions are an extra fee. Contact a trainer directly to chat or arrange a session.</p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {trainers.map((trainer, index) => (
            <Card key={index} className="bg-gray-100 rounded-lg overflow-hidden shadow-md">
              ...
              <CardContent className="p-6">
                ...
                <p className="italic mb-3">"{trainer.quote}"</p>
                ...
                {trainer.phone && (
                  <div className="flex items-center mb-1">
                    ...
                    <a href={`tel:${trainer.phone}`} className="hover:text-primary transition duration-300">{trainer.phone}</a>
                  </div>
                )}
                ...
                {trainer.email && (
                  <div className="flex items-center">
                    ...
                    <a href={`mailto:${trainer.email}`} className="hover:text-primary transition duration-300 break-all">{trainer.email}</a>
                  </div>
                )}
                
                {!trainer.phone && !trainer.email && (
                  <div className="mt-4 pt-4 border-t border-gray-200 flex justify-center">
                    <a href="#contact" className="inline-flex items-center justify-center bg-primary hover:bg-opacity-90 text-white font-medium py-2 px-4 rounded transition duration-300">
